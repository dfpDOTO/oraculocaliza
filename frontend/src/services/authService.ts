...
export async function login(username: string, password: string): Promise<Profile> {
  ...
  if (deviceError) {
    // Problema de INFRAESTRUTURA na checagem (não é "limite atingido" de
    // verdade) — em vez de bloquear a pessoa por causa de uma instabilidade,
    // deixamos ela entrar mesmo assim e avisamos no console. Bloquear um
    // usuário legítimo é pior do que, raramente, deixar passar 1 aparelho
    // a mais por causa de uma falha temporária de rede.
    console.warn('Checagem de dispositivo falhou (deixando entrar mesmo assim):', deviceError.message);
  } else {
    ...
    // só um booleano em vez de {allowed, reason}) — mesma lógica: avisa no
    // console, mas não bloqueia o acesso por causa disso.
    if (deviceResult === null || deviceResult === undefined || typeof deviceResult !== 'object') {
      console.warn('Resposta inesperada da checagem de dispositivo (deixando entrar mesmo assim).');
