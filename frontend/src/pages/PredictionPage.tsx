...
export default function PredictionPage({ resetKey }: Props) {
  ...
  const [tankLevel, setTankLevel] = useState<number>(8);
  ...
  const [errors, setErrors] = useState<{ vehicle?: string; destination?: string }>({});
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const outboundRoute = await calculateRoute([cdbriLocation, postoLocation, destinationLocation]);
            ...
            <Grid item xs={12} md={5}>
              <FormControl fullWidth>
                ...
                <Typography variant="caption" color="text.secondary" sx={{ mt: 0.5, ml: 1.5 }}>
                  Informe aqui o que você observa no painel do veículo (usado nos cálculos da viagem).
                </Typography>
              </FormControl>
            </Grid>
