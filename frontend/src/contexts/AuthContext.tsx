  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // Nunca deixa a tela de carregamento travada para sempre por causa
        // de uma falha de rede/conexão com o Supabase.
        console.error('Erro ao inicializar o app:', err);
        const isNetworkError = err instanceof TypeError && /fetch/i.test(err.message);
        setBootstrapError(
          isNetworkError
            ? 'Não foi possível conectar ao servidor. Verifique sua internet ou se um firewall/antivírus está bloqueando o acesso a "supabase.co".'
            : 'Erro ao inicializar o sistema. Tente recarregar a página.'
        );
      } finally {
        setLoading(false);
      }
